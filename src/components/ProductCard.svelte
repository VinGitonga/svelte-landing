<script lang="ts">
	import type { IProductFeature } from "../types/Product";

	export let index: number;
	export let productFeature: IProductFeature;
</script>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14 lg:gap-20 relative text-base">
	<div class={"flex flex-col gap-8 md:gap-10 pt-8 sm:pt-10" + (index % 2 === 1 ? " md:order-2" : "")}>
		<h4 class="text-2xl sm:text-3xl md:text-4xl max-w-[1000px] w-full font-medium relative pr-10 after:absolute after:top-full after:left-0 after:w-1/5 after:h-1.5 after:mt-1 after:bg-slate-900 after:pr-1">
			<slot />
		</h4>
		<p>
		...
		<div class="flex items-center">
			<button class="specialBtnDark mr-auto font-semibold"> Try Free Today &rarr; </button>
		</div>
	</div>
